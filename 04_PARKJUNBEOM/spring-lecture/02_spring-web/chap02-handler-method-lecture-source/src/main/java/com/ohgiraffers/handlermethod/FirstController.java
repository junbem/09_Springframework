...
public class FirstController {
    ...
    // /first/regist로 보내기
    // 핸들러 메서드 바환 값이 void인 경우 요청 주소가 곧 view의 이름이 된다.
    @GetMapping("/regist")
    public void regist() {
    }
     ...
     * HttpServletRequest/Response도 매개변수에 선언해서 사용하는 것이 가능하지만
     * WebRequest가 Servlet기술에 종속적이지 않아 Spring기반의 프로젝트에서 더 자주 사용된다.
     * */
     ...
     * */
    @PostMapping("/modify")
    public String modifyMenu(@RequestParam(value="name", required = true) String modifyName, @RequestParam (value="price", defaultValue = "0") int modifyPrice, Model model) {
    ...
    /*
    * 4. @SessionAttribute
    * HttpSession을 전달받는 것도 가능하지만 Servlet 종속적이므로
