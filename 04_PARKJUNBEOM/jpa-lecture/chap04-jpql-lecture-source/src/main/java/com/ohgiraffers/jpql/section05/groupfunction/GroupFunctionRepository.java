...
public class GroupFunctionRepository {
    ...
    public long countMenuOfCategory(int categoryCode){
        ...
        return entityManager.createQuery(jpql, Long.class).setParameter("categoryCode", categoryCode).getSingleResult();
    }

    /*
    * COUNT 외의 그룹함수의 경우 경로가 행이 없을 때 null값이 반환되므로 기본자료형으로 다룰 경우 문제가 생긴다.
    * Long or Double등의 Wrapper타입으로 취급
    * */
