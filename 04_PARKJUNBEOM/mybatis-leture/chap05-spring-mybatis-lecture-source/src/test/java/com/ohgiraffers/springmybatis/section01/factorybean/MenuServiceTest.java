...
/* 단위 테스트
* Mockito를 활용하여 의존성을 모킹(Mocking) 한 후 MenuService 의 순수 비즈니스 로직을 검증
* */
...
class MenuServiceTest {
    ...
    public void testFindAllMenuByOrdableStatus_Orderable() {
        ...
        when(menuMapper.findAllMenuByOrdableStatus(orderable)).thenReturn(originalList);
        // when
        List<MenuDTO> resultList = menuService.findAllMenuByOrdableStatus(orderable);
        // then
        ...
        verify(menuMapper).findAllMenuByOrdableStatus(orderable);
    }

    /* 1. 주문 가능 상태 "N"를 전달하는 테스트 시나리오 */
    @DisplayName("주문 불가능 테스트")
    @Test
